...
export function createVoiceChatSocket(
    ...
    const initializeAudio = async () => {
        try {
            ...
            onConnectionStateChange({ connected: false, connecting: false, error: null });
            return true;
        } catch (error) {
    ...
    socket.on('user-joined-voice', async ({ userId: newUserId, username, avatar }) => {
    ...
    socket.on('disconnect', () => {
        onConnectionStateChange({ connected: false, connecting: false, error: 'Disconnected' });
    });
