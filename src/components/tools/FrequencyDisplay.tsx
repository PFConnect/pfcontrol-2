...
const FrequencyDisplay: React.FC<FrequencyDisplayProps> = ({
   ...
   useEffect(() => {
      ...
      const loadFrequencies = async () => {
         ...
         const airportFreq = freqData.find(
            (f: Airport) => f.icao === airportIcao
         );
         setFrequencies(
            Array.isArray(airportFreq?.frequencies)
               ...
               <div className="flex items-center justify-between mb-2">
                  ...
                  {hasMoreFrequencies && (
                     <div className="flex items-center gap-1 text-xs text-gray-400">
                        <span>
                           +{allFrequencies.length - frequencies.length} more
                        </span>
                        {isTableOpen ? (
                  ...
                  <div className="max-h-48 overflow-y-auto">
                     <table className="w-full text-sm">
                        <thead className="bg-gray-700 sticky top-0">
                           <tr>
                              <th className="text-left px-3 py-2 text-xs">
                                 Station
                              </th>
                              <th className="text-right px-3 py-2 text-xs">
                                 Frequency
                              </th>
                           </tr>
                        </thead>
   ...
   return (
      <div className="relative">
         <div
            className={`flex items-center gap-3 rounded border border-gray-700 px-4 py-3 bg-gray-900 ${
               hasMoreFrequencies
