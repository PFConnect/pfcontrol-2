			...
			try {
				...
				if (sessionData?.atis) {
					...
					if (
						...
						sessionData.atis.letter &&
						sessionData.atis.text
					) {
							...
							while ((match = pattern.exec(atisText)) !== null) {
								...
								runways.forEach((runway) => {
									if (
										...
										!extractedLandingRunways.includes(
											runway
										)
									) {
						...
						departurePatterns.forEach((pattern) => {
							...
							while ((match = pattern.exec(atisText)) !== null) {
								...
								runways.forEach((runway) => {
									if (
										availableRunways.includes(runway) &&
										!extractedDepartingRunways.includes(
											runway
										)
									) {
	...
	const handleGenerateATIS = async () => {
		...
		try {
			const formatApproaches = () => {
				...
				const lastApproach =
					selectedApproaches[selectedApproaches.length - 1];
							...
							{availableRunways.map((runway) => (
								...
								>
									...
									<div className="flex gap-2">
										<Checkbox
											checked={landingRunways.includes(
												runway
											)}
											onChange={() =>
										...
										/>
										<Checkbox
											checked={departingRunways.includes(
												runway
											)}
											onChange={() =>
												toggleRunway(
