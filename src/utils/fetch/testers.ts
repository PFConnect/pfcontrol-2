...
export async function addTester(userId: string, notes: string = ''): Promise<Tester> {
...
export async function removeTester(userId: string): Promise<{ message: string; tester: Tester }> {
    return makeTesterRequest(`/${userId}`, {
        method: 'DELETE',
    });
}

export async function updateTesterSettings(settings: Partial<TesterSettings>): Promise<TesterSettings> {
