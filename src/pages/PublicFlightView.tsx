...
export default function PublicFlightView() {
	...
	// Real-time polling for active flights
	useEffect(() => {
		if (!flight) return;
	...
	return (
		<div className="min-h-screen bg-gray-950 text-white">
			{/* Header with PFConnect branding */}
			<div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 border-b-2 border-blue-800/30 py-4">
				<div className="container mx-auto max-w-6xl px-4">
					<div className="flex items-center gap-3">
						...
						<span className="text-xl font-bold text-white">PFControl Logbook</span>
						{isActive && (
							<span className="px-2 py-1 text-xs font-semibold bg-green-500/20 text-green-400 border border-green-500/50 rounded-full">
								LIVE
							</span>
						)}
					</div>
			...
			<div className="relative w-full bg-gradient-to-br from-blue-900/40 via-purple-900/30 to-gray-900/40 border-b-2 border-blue-800/30 py-6">
				...
				<div className="relative container mx-auto max-w-6xl px-4 py-8">
					<div className="flex items-start gap-4 mb-6">
						...
						<div>
							...
							<p className="text-gray-300 text-lg">
								{flight.aircraft_model || flight.aircraft_icao || 'Unknown Aircraft'}
							</p>
							{flight.discord_username && (
								<p className="text-blue-200 text-med mt-1">
									@{flight.discord_username}
									{flight.discord_discriminator && flight.discord_discriminator !== '0' && `#${flight.discord_discriminator}`}
								</p>
							)}
						</div>
					</div>
					...
					<div className="flex items-center gap-4 text-xl">
						<span className="px-4 py-2 bg-blue-500/20 rounded-lg font-mono font-bold text-blue-300">
							{flight.departure_icao}
						</span>
						<span className="text-gray-500">→</span>
						<span className="px-4 py-2 bg-purple-500/20 rounded-lg font-mono font-bold text-purple-300">
							{flight.arrival_icao}
						</span>
					</div>
				</div>
			...
			<div className="container mx-auto max-w-6xl px-4 py-8">
					...
					<div className="bg-gradient-to-br from-green-900/20 via-blue-900/20 to-purple-900/20 backdrop-blur-sm rounded-xl border-2 border-green-700/40 p-6 mb-6">
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							...
							<div className="bg-gradient-to-br from-gray-900/80 to-gray-900/40 rounded-xl p-4 border-2 border-gray-700/50">
								...
								<p className="text-xl font-bold text-white">
									{formatDuration(liveDuration ?? flight.duration_minutes, true)}
								</p>
							</div>
						</div>
					</div>
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
					<div className="bg-gray-900/50 rounded-xl p-4 border-2 border-gray-800">
						...
						<p className="text-2xl font-bold text-white">
							{formatDuration(liveDuration ?? flight.duration_minutes, isActive)}
						</p>
					</div>
					...
					<div className="bg-gray-900/50 rounded-xl p-4 border-2 border-gray-800">
						...
						<p className="text-2xl font-bold text-white">
							{flight.total_distance_nm !== null ? `${flight.total_distance_nm}` : '---'}
						</p>
					...
					<div className="bg-gray-900/50 rounded-xl p-4 border-2 border-gray-800">
						...
						<p className="text-2xl font-bold text-white">
							{flight.max_altitude_ft !== null ? `${flight.max_altitude_ft.toLocaleString()}` : '---'}
						</p>
					...
					<div className="bg-gray-900/50 rounded-xl p-4 border-2 border-gray-800">
						...
						<p className="text-2xl font-bold text-white">
							{flight.average_speed_kts !== null ? `${flight.average_speed_kts}` : '---'}
						</p>
				...
				{flight.landing_rate_fpm !== null && !isActive && (
					<div className={`${landingGrade.bg} border-2 ${landingGrade.border} rounded-xl px-8 py-5 mb-6 backdrop-blur-sm text-center ${landingGrade.glow}`}>
						<p className="text-sm text-gray-400 mb-1">Landing Rate</p>
						<p className={`text-4xl font-bold ${landingGrade.color} tracking-tight`}>
				...
				<div className="text-center text-gray-500 text-sm mt-8">
					...
					<p className="mt-2">
						<a href="https://control.pfconnect.online/logbook" className="text-blue-400 hover:text-blue-300">
