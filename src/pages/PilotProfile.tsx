...
export default function PilotProfile() {
	...
	const getIconComponent = (iconName: string) => {
		const icons: Record<string, any> = {
			Shield, Star, Wrench, Award, User, TrendingUp,
			FlaskConical, Crown, Zap, Target, Heart,
	...
	const getRoleBadge = (role: Role) => {
		...
		// Convert hex color to RGB for tailwind-style opacity
		const hexToRgb = (hex: string) => {
			...
			} : { r: 99, g: 102, b: 241 }; // Default color
		};
	...
	const activityChartData = {
		labels: profile.activityData.map(d => {
		...
		datasets: [
			{
				label: 'Flights',
				data: profile.activityData.map(d => parseInt(d.flight_count.toString())).reverse(),
			...
			{
				label: 'Flight Hours',
				data: profile.activityData.map(d => Math.round(parseInt(d.total_minutes.toString()) / 60)).reverse(),
	...
	return (
		<div className="min-h-screen bg-gray-950 text-white pb-12">
				...
				<div className="container mx-auto max-w-6xl px-4">
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<span className="text-xl font-bold text-white">Pilot Profile</span>
						</div>
			...
			{/* Profile Header */}
			<div className="relative w-full bg-gradient-to-br from-blue-900/40 via-purple-900/30 to-gray-900/40 border-b-2 border-blue-800/30 py-12">
				...
				<div className="relative container mx-auto max-w-6xl px-4">
					<div className="flex items-center gap-6">
						...
						<div className="relative">
							<div className="w-32 h-32 rounded-full border-4 border-blue-500/50 overflow-hidden bg-gray-800 shadow-xl shadow-blue-500/20">
								<img
						...
						<div className="flex-1">
							<h1 className="text-5xl font-bold text-white mb-2">
								...
								{profile.user.discriminator && profile.user.discriminator !== '0' && (
									<span className="text-gray-400">#{profile.user.discriminator}</span>
								)}
							</h1>
							{(profile.user.is_admin || (profile.user.roles && profile.user.roles.length > 0)) && (
								<div className="flex flex-wrap gap-2 mb-3">
									...
									{profile.user.roles && profile.user.roles.map((role) => {
							...
							<div className="flex items-center gap-2 text-gray-400">
								...
								<span className="text-sm">
									Member since {new Date(profile.user.member_since).toLocaleDateString('en-US', {
										month: 'long',
										year: 'numeric'
									})}
								</span>
						...
						<button
								...
								className="flex items-center gap-2 px-4 py-2 bg-blue-500/20 hover:bg-purple-800/30 border border-purple-500/50 rounded-lg transition-all hover:scale-105"
								title="Share profile"
							>
			...
			<div className="container mx-auto max-w-6xl px-4 mt-8">
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
					...
					<div className="bg-gradient-to-br from-purple-900/30 to-purple-800/20 rounded-xl p-6 border-2 border-purple-700/40">
						...
						<p className="text-3xl font-bold text-white">
							{profile.stats.total_hours ? `${Math.round(profile.stats.total_hours)}h` : '0h'}
						</p>
					</div>
					...
					<div className="bg-gradient-to-br from-green-900/30 to-green-800/20 rounded-xl p-6 border-2 border-green-700/40">
						...
						<p className="text-3xl font-bold text-white">
							{profile.stats.total_distance_nm ? `${Math.round(profile.stats.total_distance_nm)}` : '0'}
						</p>
					...
					<div className="bg-gradient-to-br from-yellow-900/30 to-yellow-800/20 rounded-xl p-6 border-2 border-yellow-700/40">
						...
						<p className="text-3xl font-bold text-white">
							{profile.stats.best_landing_rate ? `${Math.abs(profile.stats.best_landing_rate)}` : '---'}
						</p>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
					...
					<div className="bg-gray-900/50 rounded-xl p-4 border-2 border-gray-800">
						...
						<p className="text-xl font-bold text-white">
							{profile.stats.highest_altitude ? `${profile.stats.highest_altitude.toLocaleString()} ft` : 'N/A'}
						</p>
					</div>
				</div>
				...
				{profile.activityData.length > 0 && (
					<div className="mb-8">
						<h2 className="text-2xl font-bold text-white mb-4">Flight Activity</h2>
				...
				{profile.recentFlights.length > 0 && (
					<div>
						<h2 className="text-2xl font-bold text-white mb-4">Recent Flights</h2>
											...
											<div className="flex-1">
												<div className="flex items-center gap-3 mb-2">
													...
													<span className="text-sm text-gray-400">
														{flight.aircraft_model || flight.aircraft_icao || 'Unknown'}
													</span>
												</div>
												<div className="flex items-center gap-4 text-sm text-gray-400">
													...
													{flight.total_distance_nm && (
														<>
															<span>•</span>
															<span>{Math.round(flight.total_distance_nm)} nm</span>
														</>
													)}
												</div>
											</div>
											<div className="flex items-center gap-4">
												{flight.landing_rate_fpm && (
													<div className="text-right">
														...
														<p className={`text-sm font-semibold ${landingGrade.color}`}>
												...
												<div className="text-right">
													<p className="text-xs text-gray-500">
														{new Date(flight.flight_end).toLocaleDateString('en-US', {
															month: 'short',
															day: 'numeric'
														})}
													</p>
