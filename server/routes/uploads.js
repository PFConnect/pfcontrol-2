...
async function deleteOldImage(url) {
    if (!url) return;
    try {
        const response = await fetch(CEPHIE_DELETE_URL, {
            method: 'DELETE',
            headers: {
...
router.post('/upload-background', requireAuth, upload.single('image'), async (req, res) => {
    try {
        ...
        console.error('Error uploading background image:', error?.response?.data || error);
...
router.delete('/delete-background', requireAuth, async (req, res) => {
    try {
        const userId = req.user.userId;
