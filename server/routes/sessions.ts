...
router.post('/create', sessionCreationLimiter, requireAuth, async (req: Request, res: Response) => {
    try {
        const user = req.user;
...
router.post('/delete-oldest', requireAuth, async (req: Request, res: Response) => {
    try {
        const user = req.user;
        if (!isJwtPayloadClient(user)) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (!oldestSession) {
            return res.status(404).json({ error: 'No sessions with valid created_at found' });
        }
